...
const displayName = APP_LAUNCHER_EXPANDABLE_SECTION;
 ...
 */
const AppLauncherExpandableSection = (props) => {
	const [isOpenState, setIsOpenState] = useState(true);

	checkProps(APP_LAUNCHER_EXPANDABLE_SECTION, props, componentDoc);

	const toggleOpen = (event, data) => {
		if (props.onToggleOpen) {
			props.onToggleOpen(event, data);
		} else {
	...
	const expandableSectionProps = {
		...props,
		...{
			isOpen: props.isOpen !== undefined ? props.isOpen : isOpenState,
			onToggleOpen: toggleOpen,
		},
	};
	let ulChildrenType = 'tiles';

	const ulContent = React.Children.map(props.children, (child) => {
		let liClasses =
...
AppLauncherExpandableSection.propTypes = propTypes;
AppLauncherExpandableSection.displayName = displayName;
